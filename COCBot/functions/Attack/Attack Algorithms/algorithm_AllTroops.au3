...
Func algorithm_AllTroops() ;Attack Algorithm for all existing troops
	...
	If $g_aiAttackStdDropSides[$g_iMatchMode] <> 4 Then SmartAttackStrategy($g_iMatchMode) ; detect redarea first to drop any troops ; FourFinger Classic
	...
	$g_iSlotsGiants = 1
	If Number($GiantComp) > 16 Or (Number($GiantComp) >= 8 And $nbSides = 5) Then $g_iSlotsGiants = 2 ; will be split in 2 slots, when >16 or >=8 with FF
	If Number($GiantComp) > 20 Or (Number($GiantComp) >= 12 And $nbSides = 5) Then $g_iSlotsGiants = 0 ; spread on vector, when >20 or >=12 with FF
