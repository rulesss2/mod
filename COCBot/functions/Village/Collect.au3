...
Func Collect($Treasury = True)
	...
	; Proceeds with the second picture
	If IsArray($res) Then
		If $g_iDebugSetlog = 1 Then SetLog("DLL Call succeeded " & $res[0], $COLOR_ERROR)
		If $res[0] = "0" Or $res[0] = "" Then
			;SetLog("No Loot Cart found, Yard is clean![1]", $COLOR_SUCCESS)
			$LootCart = @ScriptDir & "\imgxml\Resources\LootCart\loot_cart_0_82.png"
			$res = DllCallMyBot("FindTile", "handle", $g_hHBitmap2, "str", $LootCart, "str", $fullCocAreas, "Int", $MaxReturnPoints)
			If @error Then _logErrorDLLCall($g_sLibImgLocPath, @error)
		EndIf
	EndIf
