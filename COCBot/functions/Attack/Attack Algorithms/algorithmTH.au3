...
Func CheckOneStar($DelayInSec = 0, $Log = True, $CheckHeroes = True)
	...
	For $i = 0 To $DelayInSec
		...
		If $CheckHeroes = True And ($g_bCheckQueenPower Or $g_bCheckKingPower or $g_bCheckWardenPower) Then CheckHeroesHealth() ;Check Heroes Health and activate their abilities if health is not green
		;check for one star
