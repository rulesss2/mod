	...
	If $g_ichkAutoUpgrade = 1 Then ; check if AutoUpgrade is enabled
		...
		If $g_iFreeBuilderCount <> 0 Then ; check free builders
			If ($g_bUpgradeWallSaveBuilder = 1 And $g_iFreeBuilderCount > 1) Or $g_bUpgradeWallSaveBuilder = 0 Then ; check if Save builder for walls is active
				SetLog("Starting Pico Auto Upgrade...", $COLOR_BLUE)
				...
				clickUpgrade()
				updateStats()
				SetLog("Pico Auto Upgrade Finished !", $COLOR_BLUE)
				randomSleep(800)
			...
			EndIf
		Else
			SetLog("No builder available, skipping AutoUpgrade...", $COLOR_WARNING)
		EndIf
	Else
		Return
	EndIf
	...
	While $canContinueLoop
		...
		If $g_iTotalBuilderCount >= 1 Then
			...
			If $g_iFreeBuilderCount <> 0 Then ; check free builders
				...
				EndIf
			Else
				SetLog("No builder available, skipping Pico Auto Upgrade...", $COLOR_WARNING)
				Return
			EndIf
			PureClickP($aAway, 1, 0, "#0133") ;Click away
			randomSleep(1500)
...
Func searchZeros() ; check for zeros on the builers menu - translate upgrade available
	...
	If QuickMIS("BC1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Price", 150, 70, 500, 300) Then
...
Func locateUpgrade() ; search for the upgrade building button
	...
	If QuickMIS("BC1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Upgrade", 200, 600, 700, 700) Then
		Return True
	Else
...
Func getUpgradeInfo()
	...
	Local $Result = QuickMIS("N1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Type", 350, 500, 740, 600)
...
Func updateAutoUpgradeLog()
	...
	If $g_sUpgradeResource = 1 Then
		...
		_FileWriteLog($g_sProfileLogsPath & "\PicoAutoUpgradeHistory.log", "Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Gold - Duration : " & $g_sUpgradeDuration)
		NotifyPushToBoth("Pico Auto Upgrade : Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Gold - Duration : " & $g_sUpgradeDuration)
	ElseIf $g_sUpgradeResource = 2 Then
		...
		_FileWriteLog($g_sProfileLogsPath & "\PicoAutoUpgradeHistory.log", "Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Elixir - Duration : " & $g_sUpgradeDuration)
		NotifyPushToBoth("Pico Auto Upgrade : Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Elixir - Duration : " & $g_sUpgradeDuration)
	ElseIf $g_sUpgradeResource = 3 Then
		...
		_FileWriteLog($g_sProfileLogsPath & "\PicoAutoUpgradeHistory.log", "Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Dark Elixir - Duration : " & $g_sUpgradeDuration)
		NotifyPushToBoth("Pico Auto Upgrade : Upgrading " & $g_aUpgradeName[1] & "from level " & $g_aUpgradeName[2] & " to level " & $g_aUpgradeName[2] + 1 & " for " & _NumberFormat($g_iUpgradeCost) & " Dark Elixir - Duration : " & $g_sUpgradeDuration)
	EndIf
